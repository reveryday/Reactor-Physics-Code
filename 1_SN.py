...
mus, weights = roots_legendre(N_angles) # 生成N_angles维高斯-勒让德求积点和权重
...
# 内迭代：裂变源固定-更新标量通量
def inner_iteration(source_fission):
...
# 外迭代：更新 k_eff
def outer_iteration(): 
    print(f"开始计算: S{N_angles}, 网格数={N_mesh}, 尺寸 a={a} cm")
    ...
    for outer_it in range(max_outer_iter):
        ...
        inner_iteration(source_fission) # 内迭代更新标量通量
