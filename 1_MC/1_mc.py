...
# 统计中子路径长度到网格通量-只统计到中点位置所在的表格（可改进）
def tally_track_length(x_start, x_end, d, dx_proj, flux_tally):  # d是沿着飞行方向的飞行距离，dx_proj是投影到x距离
    ...
    if 0 <= bin_idx < N_BINS: # 确保索引在有效范围内
        flux_tally[bin_idx] += d_eff   # 累计通量到对应网格
...
def transport_one_generation(source_bank, is_active, flux_tally):
    ...
    for x in source_bank:
        ...
        while alive:          
            ...
            d = -np.log(np.random.random()) / St #  采样飞行距离 d
            dx_proj = d * mu
            x_new = x + dx_proj
...
def resample_source(next_gen_bank, n_particles):
    if len(next_gen_bank) == 0:
        return [] 
        
    if len(next_gen_bank) > n_particles:
        return np.random.choice(next_gen_bank, n_particles, replace=False)
    else:
        return np.random.choice(next_gen_bank, n_particles, replace=True)

def run_monte_carlo():
    
    flux_tally = np.zeros(N_BINS) # 初始化通量
    k_history = []
    current_source = np.random.uniform(0, WIDTH, N_PARTICLES) # 初始源
    ...
    for gen in range(total_gens):
        ...
        if len(next_source) == 0:
            ...
            break
        current_source = resample_source(next_source, N_PARTICLES) # 源重采样 


    avg_k = np.mean(k_history)
    std_k = np.std(k_history)
    norm_factor = DX * N_ACTIVE * N_PARTICLES # 通量归一化
